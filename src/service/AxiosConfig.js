...
export const axiosDefaultRequest = axios.create({
    baseURL:API_ORIGIN,
    headers: {
        "Content-Type": "application/json",
    }
...
export const refreshAccessToken = async() => {
    const response = await axios.post("/api/access-token", {}, {
...
axiosAuthRequest.interceptors.request.use((config) => {
...
axiosAuthRequest.interceptors.response.use(
    ...
    async (error) => {
      ...
      //response가 401고, _retry가 false 라면(재요청한 적이 없다면) 리트라이
      if (error.response?.status === 401 && !originalRequest._retry) { 
        ...
        try {
          ...
          return axiosAuthRequest(originalRequest);
